...
def login_user():
  ...
  while attempts < max_attempts:
    ...
    cursor.execute("SELECT salt, password FROM users WHERE username=?",
                   (username, ))
    ...
    if user_data:
      ...
      hashed_password_to_check = bcrypt.hashpw(password.encode('UTF-8'),
                                               stored_salt)
      """
...
def retrieve_password(username):
  ...
  if user_id is not None:
    ...
    cursor.execute(
        "SELECT website, password FROM passwords WHERE user_id=? AND website=?",
        (user_id, website))
...
def validate_password(password):
  ...
  missingRequirements = [
  ]  # Array used to detect number and type of missing requirements

  # Checking each character in the password for the requirements
  ...
  if validLower and validUpper and validNums and validSpecials and validLength:
    return True

  # If not then printing the missing requirements
  else:
...
def requirements_message(missingRequirements):
  ...
  if len(missingRequirements) > 1:
    for i in range(len(missingRequirements) - 1):
      errorOutput = construct_requirements_message(missingRequirements[i],
                                                   errorOutput)
    errorOutput = errorOutput + " and"
  errorOutput = construct_requirements_message(
      missingRequirements[len(missingRequirements) - 1], errorOutput)
  print(errorOutput + '\n')


# Takes in the missing requirement and the current error message
# Appends the selected missing requirement to the error message and returns it
def construct_requirements_message(missingRequirement, errorOutput):
  # Missing requirement messages
  invalidLower = ", a lower case letter"
  invalidUpper = ", an upper case letter"
  invalidNum = ", a number"
  invalidSpecial = ", a special character"

  # Checking for which message needs to be appended
...
if __name__ == "__main__":
  ...
  cursor.execute(
      "CREATE TABLE IF NOT EXISTS passwords (id INTEGER PRIMARY KEY, user_id INTEGER, website TEXT, password TEXT)"
  )
